...
`nvim-cmp`：补全引擎，提供自动补全功能。

`vim-vsnip`：snippet 引擎，用于自动插入代码模板。

`cmp-vsnip`：补全源之一，提供与 `vim-vsnip` 集成的自动补全功能。

`cmp-nvim-lsp`：补全源之一，提供与 LSP（Language Server Protocol）集成的自动补全功能。

`cmp-buffer`：补全源之一，提供从当前缓冲区中补全的功能。

`cmp-path`：补全源之一，提供从当前路径中补全的功能。

`cmp-cmdline`：补全源之一，提供从命令行中补全的功能。

`friendly-snippets`：常见编程语言代码段集合，用于快速插入代码模板。
